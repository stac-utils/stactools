...
def test_add_asset_move(tmp_item_path: str, tmp_asset_path: str) -> None:
    """Test adding an asset to an item and moving it to the item"""
    item = pystac.Item.from_file(tmp_item_path)
    asset = pystac.Asset(
        tmp_asset_path,
        "test",
        "placeholder asset",
    ...
    assert os.path.dirname(asset_absolute_href) == os.path.dirname(item_self_href)
